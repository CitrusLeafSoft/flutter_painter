...
//    if (drawable.locked) return;
  ...
  void onDrawableScaleEnd(MapEntry<int, ObjectDrawable> entry) {
    ...
    if (drawable is LineDrawable && drawable.isGridLine) {
  ...
  void onDrawableScaleUpdate(MapEntry<int, ObjectDrawable> entry, ScaleUpdateDetails details) {
    ...
    print("old position:" + drawable.position.toString());

    if (drawable is LineDrawable && drawable.isGridLine) {
      if (details.focalPointDelta.dx > 0) {
        if (!drawable.canMoveHorizontally) return;
      } else if (details.focalPointDelta.dy > 0) {
        if (!drawable.canMoveVertically) return;
      }
    }
    ...
    final assistedRotation = assists.contains(ObjectDrawableAssist.rotation) && closestAssistAngle != null
        ...
        : rotation;
    print("new position:" + drawable.position.toString());
    if (drawable is LineDrawable && drawable.isGridLine) {
      updateDrawable(drawable, drawable);
      return;
    }
